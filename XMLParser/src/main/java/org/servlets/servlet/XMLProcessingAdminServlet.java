package org.servlets.servlet;

import org.dao.accountDAO.BankAccountDAO;
import org.dao.accountDAO.DBCPDataSource;
import org.factory.ParserFactory;
import org.parsers.DOMParser;
import org.parsers.Parser;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.sql.SQLException;

/**
 * Created by MadYeti on 07.04.2020.
 */
@WebServlet(name = "/XMLProcessingAdminServlet", urlPatterns = "/XMLProcessingAdminServlet")
public class XMLProcessingAdminServlet extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
        boolean result = false;
        String fileId = httpServletRequest.getParameter("fileid");
        String operation = httpServletRequest.getParameter("operation");
        String parserParameter = httpServletRequest.getParameter("parser");
        Parser parser = ParserFactory.createParser(parserParameter);
        BankAccountDAO bankAccountDAO = null;
        try {
            bankAccountDAO = new BankAccountDAO(DBCPDataSource.getConnection());
        } catch (SQLException e) {
            e.printStackTrace();
        }
        switch (operation){
            case "create":
                try {
                    result = bankAccountDAO.create(parser.parseXMLElement(Integer.parseInt(fileId)));
                }catch (NumberFormatException e) {
                    break;
                }
                break;
            case "read":
                if(fileId.equals("all")){
                    parser.createXMLList(bankAccountDAO.getBankAccountList());
                }else {
                    try {
                        parser.createXMLElement(bankAccountDAO.read(Integer.parseInt(fileId)), Integer.parseInt(fileId));
                    }catch (NumberFormatException e) {
                        break;
                    }
                }
                result = true;
                break;
            case "update":
                try {
                    result = bankAccountDAO.update(parser.parseXMLElement(Integer.parseInt(fileId)));
                }catch (NumberFormatException e) {
                    break;
                }
                break;
            case "delete":
                try {
                    result = bankAccountDAO.delete(parser.parseXMLElement(Integer.parseInt(fileId)));
                }catch (NumberFormatException e) {
                    break;
                }
                break;
            default:
                break;
        }
        httpServletRequest.setAttribute("result", result);
        RequestDispatcher requestDispatcher = httpServletRequest.getRequestDispatcher("/WEB-INF/view/admin.jsp");
        requestDispatcher.forward(httpServletRequest, httpServletResponse);
    }

    @Override
    protected void doPost(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
        doGet(httpServletRequest, httpServletResponse);
    }
}
